...
private:
    ...
    std::array<int,5> card_kind_sort;
  ...
  PokerHand (const char* pokerhand) {
    ...
    for (int i=0;i<5;i++){
      switch (pokerhand[i*3]){
        ...
        case 'A':   card[i]=14;
                    break;
        default:    card[i]=pokerhand[i*3]-48;
                    break;
      }
      kind_count[card[i]]++;
    ...
    //sort by kind
    for (int i=0;i<5;i++){
      card_kind_sort[i] = card[i]+kind_count[card[i]]*100;
    }
    std::sort(card_kind_sort.begin(),card_kind_sort.end(),std::greater<int>());
    for (int i=0;i<5;i++){
      card_kind_sort[i]=card_kind_sort[i]%100;
    }
    ...
    std::cout<<std::endl<<"card_kind_sort"<<std::endl; 
    for (int i=0;i<5;i++){
       std::cout<<card_kind_sort[i]<<", ";
    }
    std::cout<<std::endl<<"kind_count"<<std::endl;  
    for (int i=0;i<15;i++){
      std::cout<<kind_count[i]<<", ";
    }
     std::cout<<std::endl<<"kind_count_sort"<<std::endl;  
    for (int i=0;i<15;i++){
